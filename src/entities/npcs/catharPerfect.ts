...
class CatharPerfect extends Character {
    ...
    constructor(config: Params<EntityNode>, room?: Room) {
        super(config, getSpriteById(3, PenColor.Blue), room);
        this.inbound = false;
        this.spawning = false;
    }
    ...
    getLookingDirection(): number {
        return Math.sign(Player.getInstance().position.x - this.world.x);
    }
}
